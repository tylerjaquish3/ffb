...
$result = $conn->query("SELECT year FROM rosters ORDER BY year DESC LIMIT 1");
while ($row = $result->fetchArray()) {
    $season = $row['year'];
}

$result = $conn->query("SELECT MAX(WEEK) AS maxweek FROM rosters WHERE YEAR = $season");
while ($row = $result->fetchArray()) {
    $week = $row['maxweek'];
}
...
function saveUserActivity($pageName)
{
    ...
    try {
        ...
        $result = $conn->query("SELECT manager_id from ip_addresses WHERE ip_address = '".$ipAddress."'");
        while ($row = $result->fetchArray()) {
...
function getDashboardNumbers()
{
    ...
    $result = $conn->query("select count(distinct(year)) as num_years from finishes");
    while ($row = $result->fetchArray()) {
        $response['seasons'] = $row['num_years'];
    }

    $result = $conn->query("SELECT count(distinct(manager_id)) as winners FROM finishes WHERE finish = 1");
    while ($row = $result->fetchArray()) {
        $response['unique_winners'] = $row['winners'];
    }

    $result = $conn->query("SELECT name FROM finishes JOIN managers on managers.id = finishes.manager_id WHERE finish = 1 order by year desc limit 1");
    while ($row = $result->fetchArray()) {
        $response['defending_champ'] = $row['name'];
    }

    $result = $conn->query("SELECT MAX(championships) as championships FROM (SELECT count(manager_id) as championships FROM finishes WHERE finish = 1 group by manager_id ORDER BY championships DESC LIMIT 1) as max_num");
    while ($row = $result->fetchArray()) {
        $response['most_championships_number'] = $row['championships'];
    }

    $tempName = '';
    $result = $conn->query("SELECT count(manager_id) as championships, name FROM finishes JOIN managers on managers.id = finishes.manager_id  WHERE finish = 1 GROUP BY name HAVING count(manager_id) = " . $response['most_championships_number']);
    while ($row = $result->fetchArray()) {
        if ($tempName == '') {
            $tempName = $row['name'];
        } else {
    ...
    $result = $conn->query("SELECT count(manager1_id) as wins FROM regular_season_matchups rsm WHERE manager1_score > manager2_score GROUP BY manager1_id ORDER BY count(manager1_id) DESC LIMIT 1");
    while ($row = $result->fetchArray()) {
        $response['most_wins_number'] = $row['wins'];
    }

    $tempName = '';
    $result = $conn->query("SELECT count(manager1_id) as championships, name FROM regular_season_matchups rsm JOIN managers on managers.id = rsm.manager1_id   WHERE manager1_score > manager2_score GROUP BY name HAVING count(manager1_id) = " . $response['most_wins_number']);
    while ($row = $result->fetchArray()) {
        if ($tempName == '') {
            $tempName = $row['name'];
        } else {
...
function getPostseasonChartNumbers()
{
    ...
    $result2 = $conn->query("SELECT * FROM managers");
    while ($manager = $result2->fetchArray()) {
        $response['managers'][] = $manager['name'];
        $managerId = $manager['id'];

        $ships = $appearances = $shipAppearances = 0;
        $year = 0000;
        $result = $conn->query("SELECT * FROM playoff_matchups WHERE manager1_id = $managerId OR manager2_id = $managerId");
        while ($row = $result->fetchArray()) {
            // Calc championships
            if ($row['round'] == 'Final') {
...
function getProfileNumbers()
{
    ...
    if (isset($_GET)) {
        ...
        $result = $conn->query("SELECT * FROM managers WHERE name = '" . $_GET['id'] . "'");
        while ($row = $result->fetchArray()) {
            $managerId = $row['id'];
        }

        $result = $conn->query("SELECT name, wins, losses, total, wins/total AS win_pct
            FROM managers
            JOIN (
        ...
        $wins = $losses = 0;
        $rank = 1;
        $result = $conn->query("SELECT name, IFNULL(winsTop, 0) as winsTop, winsBottom, lossesTop, lossesBottom, totalTop, totalBottom, (IFNULL(winsTop, 0)+winsBottom)/(totalTop+totalBottom) AS win_pct
            FROM managers
            LEFT JOIN (
        ...
        $rank = 1;
        $result = $conn->query("SELECT SUM(manager1_score) as total_points, manager1_id
...
function getFinishesChartNumbers()
{
    ...
    if (isset($_GET)) {
        ...
        $result = $conn->query("SELECT * FROM finishes
...
function getSeasonNumbers()
{
    ...
    if (isset($_GET)) {
        ...
        while ($row = $result->fetchArray()) {
            ...
            $pf = $pa = $wins = $losses = 0;
            $result2 = $conn->query("SELECT * FROM regular_season_matchups
...
function getRegularSeasonMatchups()
{
    ...
    $result = $conn->query("SELECT m.name as m1, l.name as m2, rsm.year, rsm.week_number, rsm.manager1_score, rsm.manager2_score
        FROM managers m
...
function getWinsChartNumbers()
{
    ...
    $result = $conn->query("SELECT DISTINCT year FROM finishes");
    while ($row = $result->fetchArray()) {
...
function getPointMargins()
{
    ...
    $backFive = $season - 5;
    $result = $conn->query("SELECT year, week_number, AVG(manager1_score) as average FROM regular_season_matchups rsm
        WHERE year > $backFive
...
function getPostseasonMatchups()
{
    ...
    $result = $conn->query("SELECT m.name as m1, l.name as m2, rsm.year, rsm.round, rsm.manager1_seed, rsm.manager2_seed, rsm.manager1_score, rsm.manager2_score
        FROM managers m
...
function getPostseasonRecord()
{
    ...
    $result = $conn->query("SELECT
        SUM(iif(ROUND = 'Final' and manager1_score > manager2_score, 1,0)) AS final_wins,
        SUM(iif(ROUND = 'Semifinal' and manager1_score > manager2_score, 1,0)) AS semi_wins,
        SUM(iif(ROUND = 'Quarterfinal' and manager1_score > manager2_score, 1,0)) AS quarter_wins,
        SUM(iif(ROUND = 'Final' and manager1_score < manager2_score, 1,0)) AS final_losses,
        SUM(iif(ROUND = 'Semifinal' and manager1_score < manager2_score, 1,0)) AS semi_losses,
        SUM(iif(ROUND = 'Quarterfinal' and manager1_score < manager2_score, 1,0)) AS quarter_losses,
        SUM(iif(manager1_score > manager2_score, 1,0)) AS wins,
        SUM(iif(manager1_score < manager2_score, 1,0)) AS losses,
        name
        ...
        UNION
        SELECT
        SUM(iif(ROUND = 'Final' and manager2_score > manager1_score, 1,0)) AS final_wins,
        SUM(iif(ROUND = 'Semifinal' and manager2_score > manager1_score, 1,0)) AS semi_wins,
        SUM(iif(ROUND = 'Quarterfinal' and manager2_score > manager1_score, 1,0)) AS quarter_wins,
        SUM(iif(ROUND = 'Final' and manager2_score < manager1_score, 1,0)) AS final_losses,
        SUM(iif(ROUND = 'Semifinal' and manager2_score < manager1_score, 1,0)) AS semi_losses,
        SUM(iif(ROUND = 'Quarterfinal' and manager2_score < manager1_score, 1,0)) AS quarter_losses,
        SUM(iif(manager2_score > manager1_score, 1,0)) AS wins,
        SUM(iif(manager2_score < manager1_score, 1,0)) AS losses,
        name
        FROM playoff_matchups
...
function getAllNumbersBySeason()
{
    ...
    $result = $conn->query("SELECT finishes.manager_id, finishes.year, managers.name as manager_name,
...
function getCurrentSeasonPoints()
{
    ...
    $result = $conn->query("SELECT manager, roster_spot, SUM(points) AS points, SUM(projected) AS projected FROM rosters r
        WHERE YEAR = $season
...
function getCurrentSeasonStats()
{
    ...
    $result = $conn->query("SELECT manager, SUM(pass_yds) AS pass_yds, SUM(pass_tds) AS pass_tds, SUM(ints) AS ints, SUM(rush_yds) AS rush_yds, SUM(rush_tds) AS rush_tds,
...
function getCurrentSeasonBestWeek()
{
    global $conn, $season;
    $bestWeek = [];
    $result = $conn->query("SELECT week, MAX(IIF(roster_spot='QB', points, NULL)) AS top_qb,
...
function queryBestWeekPlayer($week, $pts, $pos)
{
    ...
    $result = $conn->query("SELECT * FROM rosters WHERE week = $week AND points = $pts and roster_spot = '$pos'");
    while ($row = $result->fetchArray()) {
        $response = [
            'manager' => $row['manager'],
            'player' => ($row['player']),
...
function getCurrentSeasonStatsAgainst()
{
    ...
    $result = $conn->query("SELECT year, week_number, name, manager2_id FROM regular_season_matchups rsm
    ...
    while ($row = $result->fetchArray()) {
        ...
        $result2 = $conn->query("SELECT manager, SUM(pass_yds) AS pass_yds, SUM(pass_tds) AS pass_tds, SUM(ints) AS ints, SUM(rush_yds) AS rush_yds, SUM(rush_tds) AS rush_tds,
            SUM(receptions) AS rec, SUM(rec_yds) AS rec_yds, SUM(rec_tds) AS rec_tds, SUM(fumbles) AS fum, SUM(fg_made) AS fg_made, SUM(pat_made) AS pat_made,
            SUM(def_sacks) AS def_sacks, SUM(def_int) AS def_int, SUM(def_fum) AS def_fum
            FROM rosters r
...
function getCurrentSeasonTopPerformers()
{
    ...
    $result = $conn->query("SELECT * FROM rosters WHERE YEAR = $season ORDER BY points DESC LIMIT 1");
    while ($row = $result->fetchArray()) {
    ...
    $result = $conn->query("SELECT manager, (SUM(pass_tds)+SUM(rush_tds)+SUM(rec_tds)) AS total_tds
        FROM rosters
    ...
    $result = $conn->query("SELECT manager, (SUM(pass_yds)+SUM(rush_yds)+SUM(rec_yds)) AS total_yds
        FROM rosters
        JOIN stats ON stats.roster_id = rosters.id
        WHERE YEAR = $season
        GROUP BY manager
    ...
    $result = $conn->query("SELECT manager, SUM(points) AS bench_pts
        FROM rosters
        WHERE YEAR = $season AND roster_spot = 'BN'
        GROUP BY manager
...
function getDraftedPoints($dir, $round)
{
    ...
    $result = $conn->query("SELECT rosters.manager, sum(points) as points FROM rosters
...
function getDraftPoints()
{
    ...
    $retained = [];
    $result = $conn->query("SELECT manager, COUNT(rosters.player) as players FROM rosters
        JOIN managers ON rosters.manager = managers.name
        JOIN draft ON rosters.player LIKE draft.player || '%' AND managers.id = draft.manager_id AND rosters.year = draft.year
...
function getWorstDraftPicks()
{
    ...
    $result = $conn->query("SELECT rosters.manager, draft.overall_pick, draft.position, rosters.player, sum(points) AS points FROM rosters
        JOIN managers ON rosters.manager = managers.name
        JOIN draft ON rosters.player LIKE draft.player || '%' AND managers.id = draft.manager_id AND rosters.year = draft.year
...
function getMedian($pos)
{
    ...
    $result = $conn->query("SELECT position, avg(points) AS points 
...
function getRecordAgainstEveryone()
{
    ...
    $scores = [];
    $result = $conn->query("SELECT week_number, name, manager1_score FROM regular_season_matchups rsm
...
function getAllDraftedPlayerDetails()
{
    ...
    $result = $conn->query("SELECT rosters.manager, draft.overall_pick, draft.position, draft.round, rosters.player,
...
function getOptimalLineupPoints()
{
    ...
    $result1 = $conn->query("SELECT distinct week FROM rosters WHERE YEAR = $season");
    while ($week = $result1->fetchArray()) {
        $week = $week['week'];

        $result2 = $conn->query("SELECT distinct manager FROM rosters
        ...
        while ($manager = $result2->fetchArray()) {
            ...
            $result3 = $conn->query("SELECT * FROM rosters
                WHERE YEAR = $season AND week = $week and manager = '".$manager."'");
            while ($row = $result3->fetchArray()) {

                $result4 = $conn->query("SELECT * FROM regular_season_matchups
