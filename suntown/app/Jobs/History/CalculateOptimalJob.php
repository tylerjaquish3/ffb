...
class CalculateOptimalJob implements ShouldQueue
{
    ...
    {
        $players = DB::connection('ffb')->select(
    ...
    {
        $columns = collect(DB::connection('ffb')->select('PRAGMA table_info(regular_season_matchups)'))->pluck('name');
        if (!$columns->contains('manager1_optimal')) {
            DB::connection('ffb')->statement('ALTER TABLE regular_season_matchups ADD COLUMN manager1_optimal DECIMAL(12)');
        }
        if (!$columns->contains('manager2_optimal')) {
            DB::connection('ffb')->statement('ALTER TABLE regular_season_matchups ADD COLUMN manager2_optimal DECIMAL(12)');
        }
    }

    private function ensurePlayoffColumnsExist(): void
    {
        $columns = collect(DB::connection('ffb')->select('PRAGMA table_info(playoff_matchups)'))->pluck('name');
        if (!$columns->contains('manager1_optimal')) {
            DB::connection('ffb')->statement('ALTER TABLE playoff_matchups ADD COLUMN manager1_optimal DECIMAL(12)');
        }
        if (!$columns->contains('manager2_optimal')) {
            DB::connection('ffb')->statement('ALTER TABLE playoff_matchups ADD COLUMN manager2_optimal DECIMAL(12)');
        }
    }
}
