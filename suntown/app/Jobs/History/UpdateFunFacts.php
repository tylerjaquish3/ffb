...
use App\Models\History\Roster;
use App\Models\History\RegularSeasonMatchup;
use App\Models\History\PlayoffMatchup;
use App\Models\History\Finish;
use App\Models\History\Manager;
use App\Models\History\ManagerFunFact;
use App\Models\History\RecordLog;
use App\Models\History\TeamName;
use Carbon\Carbon;
...
class UpdateFunFacts implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        if ($success) {
            if ($this->isHistoricalCalculation) {
                ...
                $count = DB::connection('ffb')->table('manager_fun_facts')->count();
    ...
    private function mostPostseasonPointsFor()
    {
        ...
        $i = DB::connection('ffb')->select("SELECT managers.id, 
    ...
    private function draft()
    {
        ...
        $ones = DB::connection('ffb')->select("SELECT managers.id, COALESCE(post_count, 0) AS num1
            FROM managers
            LEFT JOIN (
    ...
    { 
        ...
        foreach ($years as $year) {
            ...
            $players = DB::connection('ffb')->select($sql);
    ...
    public function comeback()
    {
        ...
        $result = DB::connection('ffb')->table('regular_season_matchups AS m')
            ->select([
    ...
    private function wasPlayerDrafted(string $playerName, int $year): bool
    {
        ...
        $alias = DB::connection('ffb')->table('player_aliases')
    ...
    private function trackTopPerformanceByPosition(array $positions, ?int $year, int $funFactId)
    {
        ...
        foreach ($years as $yearValue) {
            ...
            $topPerformers = DB::connection('ffb')->select($query, [$yearValue, $yearValue]);
    ...
    private function trackBestSeasonByPosition(array $positions, int $funFactId)
    {
        ...
        foreach ($allSeasons as $season) {
            ...
            foreach ($positions as $position) {
                ...
                $maxPointsPerWeek = DB::connection('ffb')->select($maxPointsPerWeekQuery, [$season, $position]);
                ...
                foreach ($maxPointsPerWeek as $weekData) {
                    ...
                    $topPerformers = DB::connection('ffb')->select($topPerformersQuery, [$season, $position, $week, $maxPoints]);
    ...
    private function allPlayRecord()
    {
        ...
        // By season
        $seasonRows = DB::connection('ffb')->select("
