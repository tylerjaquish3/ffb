...
use App\Models\History\RecordLog;
use App\Models\History\ManagerFunFact;
use App\Models\History\FunFact;
...
class UpdateWeeklyRecords implements ShouldQueue
{
    ...
    protected function calculateLeaderForFunFact($funFactId, $year, $week, $updateFunFacts)
    {
        ...
        if (isset($methodMap[$funFactId])) {
            try {
                ...
                DB::connection('ffb')->beginTransaction();
                ...
                DB::connection('ffb')->statement("ALTER TABLE regular_season_matchups RENAME TO original_regular_season_matchups");
                DB::connection('ffb')->statement("ALTER TABLE playoff_matchups RENAME TO original_playoff_matchups");
                DB::connection('ffb')->statement("ALTER TABLE draft RENAME TO original_drafts");
                DB::connection('ffb')->statement("ALTER TABLE rosters RENAME TO original_rosters");
                DB::connection('ffb')->statement("ALTER TABLE finishes RENAME TO original_finishes");
                
                // Put our temporary tables in place of the actual tables
                DB::connection('ffb')->statement("ALTER TABLE temp_regular_season_matchups RENAME TO regular_season_matchups");
                DB::connection('ffb')->statement("ALTER TABLE temp_playoff_matchups RENAME TO playoff_matchups");
                DB::connection('ffb')->statement("ALTER TABLE temp_drafts RENAME TO draft");
                DB::connection('ffb')->statement("ALTER TABLE temp_rosters RENAME TO rosters");
                DB::connection('ffb')->statement("ALTER TABLE temp_finishes RENAME TO finishes");
                ...
                DB::connection('ffb')->statement("DELETE FROM manager_fun_facts WHERE fun_fact_id = {$funFactId}");
                ...
                // Using SQLite compatible table renaming syntax
                DB::connection('ffb')->statement("ALTER TABLE regular_season_matchups RENAME TO temp_regular_season_matchups");
                DB::connection('ffb')->statement("ALTER TABLE playoff_matchups RENAME TO temp_playoff_matchups");
                DB::connection('ffb')->statement("ALTER TABLE draft RENAME TO temp_drafts");
                DB::connection('ffb')->statement("ALTER TABLE rosters RENAME TO temp_rosters");
                DB::connection('ffb')->statement("ALTER TABLE finishes RENAME TO temp_finishes");
                
                DB::connection('ffb')->statement("ALTER TABLE original_regular_season_matchups RENAME TO regular_season_matchups");
                DB::connection('ffb')->statement("ALTER TABLE original_playoff_matchups RENAME TO playoff_matchups");
                DB::connection('ffb')->statement("ALTER TABLE original_drafts RENAME TO draft");
                DB::connection('ffb')->statement("ALTER TABLE original_rosters RENAME TO rosters");
                DB::connection('ffb')->statement("ALTER TABLE original_finishes RENAME TO finishes");
                
                // Clean up temp tables
                DB::connection('ffb')->statement("DROP TABLE IF EXISTS temp_regular_season_matchups");
                DB::connection('ffb')->statement("DROP TABLE IF EXISTS temp_playoff_matchups");
                DB::connection('ffb')->statement("DROP TABLE IF EXISTS temp_drafts");
                DB::connection('ffb')->statement("DROP TABLE IF EXISTS temp_rosters");
                DB::connection('ffb')->statement("DROP TABLE IF EXISTS temp_finishes");
                DB::connection('ffb')->statement("DROP TABLE IF EXISTS saved_manager_fun_facts");
                
                // Commit the transaction
                DB::connection('ffb')->commit();
                
                return $returnResult;
            } catch (\Exception $e) {
                // Something went wrong, rollback and log
                DB::connection('ffb')->rollBack();
                ...
                try {
                    ...
                    $tables = DB::connection('ffb')->select("SELECT name FROM sqlite_master WHERE type='table' AND name LIKE 'original_%'");
                    
                    if (count($tables) > 0) {
                        // Original tables exist, try to restore them
                        // Using SQLite compatible table renaming syntax
                        if (DB::connection('ffb')->getSchemaBuilder()->hasTable('original_regular_season_matchups')) {
                            DB::connection('ffb')->statement("ALTER TABLE original_regular_season_matchups RENAME TO regular_season_matchups");
                        }
                        if (DB::connection('ffb')->getSchemaBuilder()->hasTable('original_playoff_matchups')) {
                            DB::connection('ffb')->statement("ALTER TABLE original_playoff_matchups RENAME TO playoff_matchups");
                        }
                        if (DB::connection('ffb')->getSchemaBuilder()->hasTable('original_drafts')) {
                            DB::connection('ffb')->statement("ALTER TABLE original_drafts RENAME TO draft");
                        }
                        if (DB::connection('ffb')->getSchemaBuilder()->hasTable('original_rosters')) {
                            DB::connection('ffb')->statement("ALTER TABLE original_rosters RENAME TO rosters");
                        }
                        if (DB::connection('ffb')->getSchemaBuilder()->hasTable('original_finishes')) {
                            DB::connection('ffb')->statement("ALTER TABLE original_finishes RENAME TO finishes");
                        }
                    }
    ...
    protected function runTestMode()
    {
        ...
        $factResult = DB::connection('ffb')->table('manager_fun_facts')
            ->where('fun_fact_id', 1)
            ->first();
            
        if ($factResult) {
            $manager = DB::connection('ffb')->table('managers')
